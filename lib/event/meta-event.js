...
MetaEvent._decodeInternal = function (buf, delay) {
    ...
    case MetaEvent.TYPE.DEVICE_NAME:
        ...
        break;
    case MetaEvent.TYPE.MIDI_CHANNEL:
        data.channel = buffer.readUIntLE(buf, 1);
        break;
    case MetaEvent.TYPE.MIDI_PORT:
        data.port = buffer.readUIntLE(buf, 1);
        break;
    case MetaEvent.TYPE.END_OF_TRACK:
        break;
    case MetaEvent.TYPE.SET_TEMPO:
    ...
    default:
        throw new error.MIDIDecodeError(
            'Unknown MetaEvent type: ' + hex.format(type)
        );
    }
...
MetaEvent.prototype._encodeInternal = function () {
    ...
    case MetaEvent.TYPE.MIDI_CHANNEL:
        ...
        buffer.writeUIntLE(buf, 1, this.data.channel);
        break;
    case MetaEvent.TYPE.MIDI_PORT:
        buf = new Buffer(1);
        buffer.start(buf);

        buffer.writeUIntLE(buf, 1, this.data.port);
        break;
    case MetaEvent.TYPE.END_OF_TRACK:
        buf = new Buffer(0);
        break;
    ...
    case MetaEvent.TYPE.KEY_SIGNATURE:
        ...
        break;
    case MetaEvent.TYPE.SEQUENCER_SPECIFIC:
        buf = new Buffer(this.data.bytes.length);
        buffer.start(buf);

        buffer.copy(buf, this.data.bytes);
        break;
    default:
