...
/**
 * Decode given buffer of MIDI data to a channel event
 *
 ...
 */
ChannelEvent.decode = function (delay, status, buf) {
    ...
    case ChannelEvent.TYPE.NOTE_OFF:
    case ChannelEvent.TYPE.NOTE_ON:
        data.note = buf.readUIntLE(8);
        data.velocity = buf.readUIntLE(8);
        break;
    case ChannelEvent.TYPE.NOTE_AFTERTOUCH:
        data.note = buf.readUIntLE(8);
        data.pressure = buf.readUIntLE(8);
        break;
    case ChannelEvent.TYPE.CONTROLLER:
        data.type = buf.readUIntLE(8);
        data.value = buf.readUIntLE(8);
        break;
    case ChannelEvent.TYPE.PROGRAM_CHANGE:
        data.instrument = buf.readUIntLE(8);
        break;
    case ChannelEvent.TYPE.CHANNEL_AFTERTOUCH:
        data.pressure = buf.readUIntLE(8);
        break;
...
ChannelEvent.prototype.encode = function () {
    ...
    case ChannelEvent.TYPE.NOTE_ON:
        ...
        buf.writeUIntLE(8, this.data.note);
        buf.writeUIntLE(8, this.data.velocity);
        break;
    case ChannelEvent.TYPE.NOTE_AFTERTOUCH:
        buf = new MIDIBuffer(2);

        buf.writeUIntLE(8, this.data.note);
        buf.writeUIntLE(8, this.data.pressure);
        break;
    case ChannelEvent.TYPE.CONTROLLER:
        buf = new MIDIBuffer(2);

        buf.writeUIntLE(8, this.data.type);
        buf.writeUIntLE(8, this.data.value);
        break;
    case ChannelEvent.TYPE.PROGRAM_CHANGE:
        buf = new MIDIBuffer(1);

        buf.writeUIntLE(8, this.data.instrument);
        break;
    case ChannelEvent.TYPE.CHANNEL_AFTERTOUCH:
        buf = new MIDIBuffer(1);
    ...
    case ChannelEvent.TYPE.PITCH_BEND:
        ...
        buf.writeUIntLE(8, value & 0x7F);
        buf.writeUIntLE(8, value >> 7);
        break;
    }

    result = new MIDIBuffer(buf.getLength() + 1);
    result.writeUIntLE(8, (this.type << 4) + this.channel);
    result.copy(buf);
