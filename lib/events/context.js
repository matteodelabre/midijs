...
/**
 * @class Context
 * @classdesc Manipulate MIDI events in the context in which
 * they appeared. This class helps with encoding and decoding
 * events in a variety of situations, thus you should use it
 * instead of event-specific .decode and #decode methods.
 *
 * @param {Object} options Context options
 * @param {bool} [options.device=false] Whether events come or go to a device
 * @param {bool|Object} [options.runningStatus=true] Enable running status?
 */
...
/**
 * Reset the buffer that keeps track of the last status, meaning that
 * subsequent events will not use previous statuses
 *
 * @return {null}
 */
...
/**
 * Decode given MIDI data to a list of events
 * in current context
 *
 * @param {MIDIBuffer|Buffer|Array|*} input Data to decode
 * @return {Array<ChannelEvent|MetaEvent|SysexEvent>} Decoded events
 */
...
/**
 * Encode one or more events to a buffer
 *
 * @param {Event|Array<Event>} events Event(s) to encode
 * @return {Buffer|Array} Array or buffer of encoded bytes
 * (will be an array only if current context is in device mode)
 */
Context.prototype.encode = function (events) {
    ...
    result = Buffer.concat(events.map(function (event) {
        var subresult = event.encode().toBuffer(),
            status = subresult[0], extended;

        // use running status if enabled and applicable
        if (this.options.runningStatus.out && status === this.lastStatus) {
            subresult = subresult.slice(1);
        } else {
            if (status >= 0xF0 && status <= 0xF7) { // 0xF0 - 0xF7 reset rs
                this.lastStatus = null;
            } else if (status < 0xF8) { // 0xF8 - 0xFF ignore running status
        ...
        if (!this.options.device) {
            extended = new MIDIBuffer(subresult.length + vintLength(event.delay));
            extended.writeVarInt(event.delay);
            extended.copy(subresult);

            subresult = extended.toBuffer();
        }

        return subresult;
    }, this));

    // device accept arrays
    if (this.options.device) {
