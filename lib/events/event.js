 ...
 * @class Event
 * @abstract
 * @classdesc This is a class that can represent any kind of
 * MIDI event. As such, you should not instanciate it directly, but use
 * child classes such as {@link ChannelEvent} or {@link SysexEvent}.
 * If you need to decode an event, you need to use {@link Event.decode}
 * because you don't know the event type beforehand
 *
 * @example <caption>Encode an event and send it to a device</caption>
 ...
 * }).encode());
 *
 * @example <caption>Decode an event incoming from a device</caption>
