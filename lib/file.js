...
File.prototype.setData = function (data, callback) {
    ...
    try {
        ...
        if (header.getFileType() === 0 && header._trackCount > 1) {
            throw new error.MIDIFileParserError(
                header._trackCount + ' tracks',
                '1 track (because of file type 0)',
                cursor.tell()
            );
        }
 ...
 *
 * @return {Array<module:midijs/lib/track~Track|undefined}
 */
