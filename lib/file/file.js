...
// access subclasses type constants
File.META = MetaEvent.TYPE;
File.SYSEX = SysexEvent.TYPE;
File.CHANNEL = ChannelEvent.TYPE;
...
File.decode = function (buf) {
    var decoded = chunk.decode(buf), data = decoded.data,
        type, tracks = [], ticksPerBeat;

    if (decoded.type !== 'MThd') {
        throw new error.MIDINotMIDIError('Not a MIDI file');
    }
    ...
    while (!buffer.eof(data)) {
        tracks.push(Track.decode(data));
    }

    // only keep first track if single mode
...
File.prototype.encode = function () {
    var header = new Buffer(6), tracks;

    buffer.start(header);
    buffer.writeUIntBE(header, 2, this.type);
    buffer.writeUIntBE(header, 2, this.tracks.length);
...
File.prototype.track = function () {
    ...
    // proxy object so that we can use the
    // cool #end() syntax
    return {
        meta: function () {
