 ...
 * @exception {module:midijs/lib/error~MIDIFileParserError}
 * Time is expressed in unsupported SMPTE format
...
function parseHeader(cursor) {
    ...
    if ((timeDivision & 0x8000) === 0) {
        ...
        throw new error.MIDIFileParserError(
            'time in SMPTE format (unsupported)',
            'time in ticks per beat format',
            cursor.tell()
        );
    }
