...
/**
 * Check that a single-track MIDI file doesn't contain
 * more than one track
 *
 * @param {File} file File to check
 * @throws {Error} If the file contains more than one track
 */
checkTracks = function (file) {
    if (file.type === File.TYPE.SINGLE_TRACK && file.tracks.length >= 2) {
        throw new Error(
            'A single-track file cannot contain more than one track'
        );
    }
};
...
File.decode = function (input) {
    ...
    try {
        ...
        throw new MalformedError('Invalid MIDI header');
    }
...
File.prototype.encode = function () {
    ...
    return MIDIBuffer.concat([chunk].concat(tracks)).toBuffer();
};
