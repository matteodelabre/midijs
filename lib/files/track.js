...
var Context = require('../events/context');
...
Track.decode = function (input) {
    var decoded = new MIDIBuffer(input).readChunk(),
        data = decoded.data, ctx;
    ...
    ctx = new Context();
    return new Track(ctx.decode(data)).end();
};
...
Track.prototype.encode = function () {
    ...
    ctx = new Context();
    data = ctx.encode(this.events);

    chunk = new MIDIBuffer(data.length + 8);
