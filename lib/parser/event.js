...
function EventMeta(cursor) {
    ...
    case 81:
        ...
        this.microsecondsPerBeat = ((cursor.readUInt8() << 16) +
                                    (cursor.readUInt8() << 8) +
                                     cursor.readUInt8());
        break;
    case 84:
    ...
    case 88:
        ...
        this.notesPerClockSignal = cursor.readUInt8();
        break;
    case 89:
        this.subtype = 'keySignature';
        this.key = cursor.readInt8();
        this.major = !cursor.readUInt8();
        break;
    case 127:
        this.subtype = 'sequencerSpecific';
        this.data = cursor.toString('ascii', length);
        break;
    }
}
...
function EventSysex(subtype, cursor) {
    ...
    // FIXME: We should read system events, or
    //        at least store them in some place
    
    this.subtype = subtype;
    this.data = cursor.slice(length);
}
 ...
 * Parse a MIDI event
 *
 * @param {lastType: int}           Last event type in the track
 * @param {cursor: BufferCursor}    Buffer to parse
 */
function Event(lastType, cursor) {
